...
namespace mapManager{
	...
	void occMap::initParam(){
		...
		// min vis height
		if (not this->nh_.getParam(this->ns_ + "/min_height_visualization", this->minVisHeight_)){
			this->minVisHeight_ = 0.0;
			cout << this->hint_ << ": No min visualization height. Use default: 0.0 m." << endl;
		}
		else{
			cout << this->hint_ << ": Min visualization height: " << this->minVisHeight_ << endl;
		}
		...
		// init pre_position_
		for (int i = 0; i < 30 ; i++) {
			this->pre_position_.push_back(kdt::MyPoint(0, 0));
    	}
	...
	void occMap::registerCallback(){
		...
		// predict point callback
		this->point_pre_sub_ = this->nh_.subscribe("/point_predict", 10, &occMap::pointCloud2CB,this); // 订阅话题：接收MPC预测信息
	}
	...
	void occMap::registerPub(){
		...
		this->static_obs_pub_ = this->nh_.advertise<sensor_msgs::PointCloud2>(this->ns_ + "/static_obs_point", 10);  // publish static risk point
		this->map2DPub_ = this->nh_.advertise<nav_msgs::OccupancyGrid>(this->ns_ + "/occupancy_2D_map", 10);
		...
		this->sysdataPub_ = this->nh_.advertise<std_msgs::Float32MultiArray>(this->ns_ + "/sys_data", 10);
	}
	...
	void occMap::depthPoseCB(const sensor_msgs::ImageConstPtr& img, const geometry_msgs::PoseStampedConstPtr& pose){
		...
		if (this->isInMap(this->position_)){
			...
			// std::cout << "z:" << this->position_(2) << endl ;
		}
		else{
		...
		for (int v=this->depthFilterMargin_; v<rows-this->depthFilterMargin_; v=v+this->skipPixel_){ // row
			...
			for (int u=this->depthFilterMargin_; u<cols-this->depthFilterMargin_; u=u+this->skipPixel_){ // column
				...
				if (*rowPtr == 0) {
					...
					depth = this->depthMinValue_ ; //  自己修改添加
					// continue;
	...
	void occMap::pointCloud2CB(const sensor_msgs::PointCloud2ConstPtr& msg){
	...
	void occMap::publishInflatedMap(){
			...
			for (int y=minRangeIdx(1); y<=maxRangeIdx(1); ++y){
				for (int z=minRangeIdx(2); z<=maxRangeIdx(2); ++z){
					...
					if (this->isInflatedOccupied(pointIdx)){
						Eigen::Vector3d point;
						this->indexToPos(pointIdx, point);
						if (point(2) <= this->maxVisHeight_ && point(2)>= this->minVisHeight_){
							pt.x = point(0);
							pt.y = point(1);
							pt.z = point(2);
							cloud.push_back(pt);
		...
		if (i){
			this->points_kdt_Search(points_kdt, this->pre_position_); // 查找每个预测点最近的障碍物点并发布
		}
	}

	// pub the obstacle points corresponding to the predicted state
	void occMap::points_kdt_Search(const std::vector<kdt::MyPoint>& points, const std::vector<kdt::MyPoint>& pre_point){
		int collision_flag = 1;
		int nnSearch_flag = 0;
		int nnSearch_flag_sec = 0;
		int nnSearch_flag_back = 1;
		int i_back = 0;
		int i_flag = 0;
		kdt::KDTree<kdt::MyPoint> kdtree(points); // kd-tree
		pcl::PointCloud<pcl::PointXYZ>::Ptr cloud(new pcl::PointCloud<pcl::PointXYZ>);
		cloud->width = 31;  
		cloud->height = 1;  
		cloud->is_dense = true;  
		...
		for (int i=0; i < pre_point.size(); i++){
			if (collision_flag && this->isOccupied(Eigen::Vector3d (pre_point[i][0], pre_point[i][1], 0.25))){
				cloud->points[i].x = (double)((int)(pre_point[i][0]/0.05)*0.05);
				cloud->points[i].y = (double)((int)(pre_point[i][1]/0.05)*0.05);
				collision_flag = 0; // first
			}
			if (!nnSearch_flag){	 
				// generate query (center of the space)
				kdt::MyPoint query(pre_point[i][0], pre_point[i][1]);
				// nearest neigbor search
				int idx = kdtree.nnSearch(query);
				if (sqrt((pre_point[i][0]-points[idx][0])*(pre_point[i][0]-points[idx][0])+(pre_point[i][1]-points[idx][1])*(pre_point[i][1]-points[idx][1])) < 0.533){
					cloud->points[i].x = points[idx][0];
					cloud->points[i].y = points[idx][1];
					nnSearch_flag++;
				}
				else if (i == 1){
					cloud->points[i].x = points[idx][0];
			...
			if(nnSearch_flag_back){
				i_back = pre_point.size() - 1 - i;
				if(i_back>i){
					kdt::MyPoint query(pre_point[i_back][0], pre_point[i_back][1]);
					int idx = kdtree.nnSearch(query);
					if (sqrt((pre_point[i_back][0]-points[idx][0])*(pre_point[i_back][0]-points[idx][0])+(pre_point[i_back][1]-points[idx][1])*(pre_point[i_back][1]-points[idx][1])) < 0.311){
						cloud->points[i_back].x = points[idx][0];
						cloud->points[i_back].y = points[idx][1];
						nnSearch_flag_back=0;
					}
				}
			}
		}
		...
		this->static_obs_pub_.publish(point_nearest_);

		// sys_data
		kdt::MyPoint query1(this->position_(0), this->position_(1));
		// nearest neigbor search
		int idx = kdtree.nnSearch(query1);
		std_msgs::Float32MultiArray sys_data;
		sys_data.data.push_back(sqrt((this->position_(0)-points[idx][0])*(this->position_(0)-points[idx][0])+(this->position_(1)-points[idx][1])*(this->position_(1)-points[idx][1]))-0.333);
		sys_data.data.push_back(this->position_(0));
		sys_data.data.push_back(this->position_(1));
	}
