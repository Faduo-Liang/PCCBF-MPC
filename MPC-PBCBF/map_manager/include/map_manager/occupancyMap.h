...
namespace mapManager{
	...
	protected:
		...
		ros::Publisher static_obs_pub_;  
		ros::Publisher map2DPub_;
		ros::Publisher mapExploredPub_;
		ros::Publisher sysdataPub_;
	...
	public:
		...
		void points_kdt_Search(const std::vector<kdt::MyPoint>& points, const std::vector<kdt::MyPoint>& pre_point);
	...
	inline bool occMap::isOccupied(const Eigen::Vector3i& idx){
		...
		return this->occupancy_[address] >= this->pOccLog_; // 判断：如果数值大于等于占用阈值，则为占用；
	}
