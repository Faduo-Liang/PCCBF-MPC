...
#include "onboard_detector/kd_tree.h"
...
namespace onboardDetector{
    ...
    private:
        ...
        std::shared_ptr<onboardDetector::DBSCAN> dbCluster_yolo;
        ...
        Eigen::Vector3d centerHist_yolo; //  存储yolo得到的bbox的位置估计的上一次结果，用于滤波
        ...
        int dbMinPointsCluster_yolo;
        double dbEpsilon_;
        double dbEpsilon_yolo;
        ...
        std::vector<Eigen::Vector3d> dynamicPoints_; // now step dynamic points
        ...
        std::vector<int> pointsFlag_yolo;
        std::vector<std::vector<Eigen::Vector3d>> projPoints_yolo; // projected points from depth image in the yolo boxs
        std::vector<std::vector<Eigen::Vector3d>> pcClusters_yolo; // pointcloud clusters
        std::vector<Eigen::Vector3d> pcClusterCenters_yolo; // pointcloud cluster centers
        std::vector<Eigen::Vector3d> pcClusterStds_yolo; // pointcloud cluster standard deviation in each axis

        // Dynamic obstacle prediction trajectory
        int N_ = 30;
        vector<double> dyn_obs_state; 
        std::vector<std::vector<Eigen::Vector3d>> dynamicPoints_predict; // predict dynamic point
        std::vector<kdt::MyPoint> pre_position_; // predict position
    ...
    public:
        ...
        void predict_obs_point_CB(const sensor_msgs::PointCloud2ConstPtr& msg);
        ...
        void clusterPointsAndBBoxes(const std::vector<std::vector<Eigen::Vector3d>>& points, std::vector<onboardDetector::box3D>& bboxes, std::vector<std::vector<Eigen::Vector3d>>& pcClusters, std::vector<Eigen::Vector3d>& pcClusterCenters, std::vector<Eigen::Vector3d>& pcClusterStds);
        ...
        void publishYoloDepthImages();
        void publishPoints(const std::vector<Eigen::Vector3d>& points, const ros::Publisher& publisher);
        void publishPoints(const std::vector<std::vector<Eigen::Vector3d>>& points, const ros::Publisher& publisher);
        ...
        void publishVelVis();
        void publishObsPredict(const std::vector<onboardDetector::box3D>& boxes, const ros::Publisher& publisher);
        ...
        void points_kdt_Search(const std::vector<std::vector<Eigen::Vector3d>>& points, const std::vector<kdt::MyPoint>& pre_position_);
    };
    ...
    inline void dynamicDetector::posToIndex(const Eigen::Vector3d& pos, Eigen::Vector3i& idx, double res){
        idx(0) = floor( (pos(0) - this->position_(0) + localSensorRange_(0)) / res); // 计算并返回小于或等于给定数字的最大整数
