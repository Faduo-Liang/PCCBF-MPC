...
namespace onboardDetector{
    ...
    {
        this->row_downsample = 3; // U_map行缩放
        this->col_scale = 0.5; // 列缩放比例 调整
        this->min_dist = 10;
        this->max_dist = 8000;// unit: m
        this->threshold_point = 3;
        this->threshold_line = 5; // 
        this->min_length_line = 10; // 划分uvbox的最小长度
        this->show_bounding_box_U = true;
        this->show_bounding_box_U = true;
        // the following intrinsic parameters can be found in /camera/../camera_info
        this->fx = 241.245043295;  
        this->fy = 242.02536811;
        this->px = 320.13062082;
        this->py = 174.93689173;
    ...
    void UVdetector::extract_U_map() 
    {
        ...
        resize(this->depth, depth_rescale, cv::Size(),this->col_scale , 1); // 对深度图this->depth进行缩放，列方向上的缩放比例为this->col_scale，行方向保持不变（缩放因子为1）。缩放后的深度图存储在depth_rescale中
        ...
        // construct the mask
        cv::Rect mask_depth;
        uint8_t histSize = this->depth.rows / this->row_downsample; // U_map 行大小

        int bin_width = ceil((this->max_dist - this->min_dist) / float(histSize)); // (8000-10)/histSize 代表U_map深度单位量
        ...
        for(int col = 0; col < depth_rescale.cols; col++)
        {
            for(int row = 0; row < depth_rescale.rows; row++)
            {
                ...
                depth_rescale_val = int((float(depth_rescale.at<unsigned short>(row, col))/this->depthScale_)*1000.0); // 将深度值按this->depthScale_进行缩放，并乘以1000.0转换为整数
                ...
                if(depth_rescale_val > this->min_dist && depth_rescale_val < this->max_dist)
                {
                    ...
                    depth_low_res_temp.at<uchar>(row, col) = bin_index; // 将深度值转换为depth_low_res_temp图（本质还是存储深度信息）
                    ...
                    if(this->U_map.at<uchar>(bin_index, col) < 255)
                    {
                        ...
                        this->U_map.at<uchar>(bin_index, col) ++; // 将相同深度值作统计，这一步实际上是在统计深度图每一列的相同深度数量，并且深度大小是连续的。
                    }
                }
            }
        }
        ...
        GaussianBlur(this->U_map, this->U_map, cv::Size(5,9), 10, 10); // 高斯模糊处理，平滑结果
        // printf("rescaled depth map: %d", depth_rescale.at<unsigned short>(testy,testx));
    }

    // 从U_map中提取出满足特定条件的区域（称为UVbox），并将这些区域合并成边界框
    void UVdetector::extract_bb()
    {
        // initialize a mask
        std::vector<vector<int> > mask(this->U_map.rows, vector<int>(this->U_map.cols, 0)); // 与U_map大小相同的向量
        // initialize parameters
        int u_min = this->threshold_point * this->row_downsample; // 3*3 最小深度数量
        int sum_line = 0;
        int max_line = 0;
        int length_line = 0;
        int seg_id = 0;
        ...
        // printf("Umap rows %d\n",this->U_map.rows);
        for(int row = 0; row < this->U_map.rows; row++) // 行代表深度值，遍历每个深度值
        {
            for(int col = 0; col < this->U_map.cols; col++)
            {
                // is a point of interest 是期望点
                if(this->U_map.at<uchar>(row,col) >= u_min) // num of points at this depth >= u_min
                {
                    // update current line info
                    length_line++; // 连续期望点的长度
                    sum_line += this->U_map.at<uchar>(row,col); // 深度数量的和
                    max_line = (this->U_map.at<uchar>(row,col) > max_line)?this->U_map.at<uchar>(row,col):max_line; // 最大深度数量
                }
                // is not or is end of row 不是期望点或是最后一列
                if(this->U_map.at<uchar>(row,col) < u_min || col == this->U_map.cols - 1)
                {
                    ...
                    {
                        seg_id++; // id
                        UVboxes.push_back(UVbox(seg_id, row, col - length_line, col - 1)); // 将满足上述要求的线加入UVboxes，（id,行,列起始点,列终止点）
                        // overwrite the mask with segementation id
                        for(int c = col - length_line; c < col - 1; c++)
                        {
                            mask[row][c] = seg_id; // 对应位置赋予id值
                        }
                        // when current row is not first row, we need to merge neighbour segementation 如果当前行不是第一行，则检查上一行的对应像素是否已被标记为某个UVbox的一部分，并尝试合并
                        if(row != 0)
                        {
                            // merge all parents
                            for(int c = col - length_line; c < col - 1; c++)
                            {
                                if(mask[row - 1][c] != 0) // 上一行对应的元素已被标记为某个UVbox的一部分
                                {
                                    // initially, toppest parent id is just seg id.
                                    // mask stores seg_ids
                                    // 如果上一行对应位置的父id < 当前行的id，说明可以合并
                                    if(UVboxes[mask[row - 1][c] - 1].toppest_parent_id < UVboxes.back().toppest_parent_id)
                                    {
                                        // 将当前行的父id全部改为上一行的id，进行合并
                                        UVboxes.back().toppest_parent_id = UVboxes[mask[row - 1][c] - 1].toppest_parent_id;
                                    }
                                    else // ？否则，更新所有具有相同父id的UVbox的父id为当前行的id。将这些UVbox的父级统一为当前UVbox的父级或自身（如果当前UVbox是最新的）
                                    {
        ...
        // group lines into boxes 将线段组合成边界框
        ...
        for(size_t b = 0; b < UVboxes.size(); b++)
        {
            if(UVboxes[b].id == UVboxes[b].toppest_parent_id)
            {
                ...
                // check box's size
                if(UVboxes[b].bb.area() >= 25) // 只保留面积大于或等于 25 的UVbox作为最终的边界框。 可调整
                {
                    this->bounding_box_U.push_back(UVboxes[b].bb);
                    // std::cout<<" UVboxes_area: "<< UVboxes[b].bb.area() <<std::endl;
                }
            }
        }
    }
    ...
    {
        // extract U map from depth 从深度图中提取U-map
        this->extract_U_map();

        // extract bounding box from U map 从U-map中提取包围框
        this->extract_bb();

        // extract bounding box 生成鸟瞰图
    ...
    {   
        ...
        for (size_t b = 0; b < this->bounding_box_U.size(); b++) {
            ...
            curr_box.x_width = im_frame_x_width*Y_w/this->fx; // 调整
    ...
    void UVdetector::display_bird_view() 
    {
        ...
        for(size_t b = 0; b < this->bounding_box_U.size(); b++)
        {
            ...
            // std::cout << "[bird_view]" << " x: " << bb_center.x << " y: " << bb_center.y << std::endl; // 像素坐标？
        }
